...
for attr_num, (attribute, (min_measurement, max_measurement)) in enumerate(column_data.items()):
    ...
    rects_1 = ax.bar(x + offset, min_measurement, width, label=labellings[0], color=colours[0])    # , color=colour_map.colors
    rects = ax.bar(x + offset, max_measurement - min_measurement, width, label=labellings[1], bottom=min_measurement, color=colours[1])
...
for i in range(len(leg.legend_handles)):
    leg.legend_handles[i].set_facecolor(colour_map.colors[i])
